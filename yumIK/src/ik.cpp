...
bool srvYumIK(yumik::YumIK::Request &req, yumik::YumIK::Response &res)
{
    Eigen::Vector3f trObj(req.pose.position.x, req.pose.position.y, req.pose.position.z);

    Eigen::Quaternionf oriObj;
    ...
    std::vector< std::vector<float> > ikYumiSol;

    ikYumiSol = ikYumi->AnalyticalIKSolver(Pose,req.theta3);
    ...
    if ( !ikYumiSol.empty() )
    {
        ...
        for(int i=0; i<ikYumiSol.size(); i++)
        {
            for(int j=0; j<ikYumiSol[i].size(); j++)
            {
                std::cout<<ikYumiSol[i][j]<<" ";
            }
            std::cout<<std::endl;
        }

        res.status = true;
        res.ikSolutions.resize(ikYumiSol.size());
        for (unsigned int i = 0; i < ikYumiSol.size(); ++i) {
            res.ikSolutions[i].ik.resize(ikYumiSol.at(i).size());
            for (unsigned int j = 0; j < ikYumiSol.at(i).size(); ++j) {
                res.ikSolutions[i].ik[j] = ikYumiSol.at(i).at(j);
            }
        }
        return true;
    }
        ...
        ROS_INFO("IK has not been computed.\n");
        res.status = false;
        return false;
    }
}
...
int main(int argc, char** argv)
{
    ...
    ros::ServiceServer service1 = n.advertiseService("Yumi/IKSolver", srvYumIK);
